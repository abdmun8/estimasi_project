...
class Ion_auth_model extends CI_Model
{
	...
	public function set_session($user)
	{
		...
		$extra = $this->db->get_where($this->tables['users'], ['id' => $user->id])->row();
		$extra_session = [
			'username' => $user->{$this->identity_column},
			'password' => $extra->password,
			'id_karyawan' => $extra->id_karyawan,
			'hak_akses' => $extra->hak_akses,
			'nama' => $extra->nama
		];
		$session_data = array_merge($session_data, $extra_session);
	...
	public function login_remembered_user()
	{
		...
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column . ', id, email, remember_code, last_login, hak_akses, id_karyawan')
	...
	/* return password */
	public function _set_encrypt($identity, $password)
	{
		$hash = $this->hash_password($password, $identity);
